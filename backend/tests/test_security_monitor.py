...
def test_ddos_detection():
    ...
    alerts = []
    for i in range(monitor.thresholds['ddos_sources'] + 1):
        src_ip = f"192.168.1.{100 + i}"
        packet = {'src_ip': src_ip, 'dst_ip': dst_ip, 'length': 64, 'protocol': 'TCP', 'dst_port': 9999, 'tcp_flags': 0, 'src_port': 0}
        alerts.extend(monitor.analyze_packet(packet))

    assert any(a.get('alert_type') == 'ddos' for a in alerts)
